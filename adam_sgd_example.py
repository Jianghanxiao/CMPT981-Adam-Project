...
def main(args):
    ...
    set_seed(args.seed)
    run_seeds = np.random.randint(1000000, size=args.runs)
    ...
    for name in optimizers:
        for seed in run_seeds:
            for lr in [1e-1]:#[1e-7, 1e-6, 1e-5, 9e-4, 3e-4, 1e-4, 9e-3, 3e-3, 1e-3, 9e-2, 3e-2, 1e-2, 0.9, 0.3, 0.1, 1.]:
                ...
                LOSSES[name][seed][lr] = losses
            

    for name in LOSSES.keys():
        for seed in LOSSES[name].keys():
            min_loss = math.inf
            min_lr = None
            for lr, lr_losses in LOSSES[name][seed].items():
                if np.mean(lr_losses) < min_loss:
                    min_loss = np.mean(lr_losses)
                    min_lr = lr
            if min_lr is not None:
                LOSSES[name][seed]['min_lr'] = LOSSES[name][seed][min_lr]
            else:
                LOSSES[name][seed]['min_lr'] = None

        seed_losses = []
        for seed in LOSSES[name].keys():
            if LOSSES[name][seed]['min_lr'] is not None:
                seed_losses.append(np.array(LOSSES[name][seed]['min_lr']))

        LOSSES[name]['mean'] = np.mean(seed_losses, axis=0)
    
    
    for name in LOSSES.keys():        
        losses = np.log10(np.array(LOSSES[name]['mean']))
